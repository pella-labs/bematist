...
| H2 | Playwright `admin-github` storageState fixture | **Landed** — `apps/web/tests/e2e/fixtures/admin.storageState.json` seeds a legacy `bema-session` cookie; middleware passes; session resolver's Redis lookup returns null and falls through to the dev-mode admin fallback |
