...
--   3. Attach `org_isolation` policy using `current_setting('app.current_org_id', true)::uuid`.
--   4. Grant SELECT/INSERT/UPDATE/DELETE on all tables to app_bematist.
--
-- Not enabled on: `orgs` (it IS the tenant table, no row-level filtering needed),
...
-- `erasure_requests` uses `target_org_id`; `policies` uses `org_id` (PK).
DO $$
DECLARE
  t text;
  col text;
