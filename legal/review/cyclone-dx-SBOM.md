...
**Maintained by:** Bematist Workstream I (Compliance), in coordination with Workstream F (release tooling — Sebastian).
...
| Bematist's own supply-chain risk management | Continuous Dependabot + diff against last-release SBOM detects malicious-update injection |
| CycloneDX vs. SPDX | Bematist uses **CycloneDX 1.5** as the canonical format; SPDX-2.3 conversion is supplied on request via `cyclonedx-cli convert`. Rationale: CycloneDX has first-class support for VEX (vulnerability-exchange) attestations and is the Cloud Native Computing Foundation graduated standard for container SBOM. |

---

## 2. Scope per artifact

Bematist ships several distinct artifacts; each gets its own SBOM:

| Artifact | Location | SBOM file |
|---|---|---|
| Bematist collector binary (Bun-compiled, single binary, per-OS-per-arch) | GitHub Release asset `bematist-<version>-<os>-<arch>` | `bematist-<version>-<os>-<arch>.cdx.json` |
| `apps/web` container image | GHCR `ghcr.io/bematist/web:<version>` | `web-<version>.cdx.json` |
| `apps/ingest` container image | GHCR `ghcr.io/bematist/ingest:<version>` | `ingest-<version>.cdx.json` |
| `apps/worker` container image | GHCR `ghcr.io/bematist/worker:<version>` | `worker-<version>.cdx.json` |
| `apps/ingest-sidecar` (Go side-car, per F15 Plan B) | GHCR `ghcr.io/bematist/ingest-sidecar:<version>` | `ingest-sidecar-<version>.cdx.json` |
| Aggregate "release" SBOM (union of all of the above + cross-references) | GitHub Release asset `bematist-<version>.cdx.json` | as listed |
...
Per the CycloneDX specification, every Bematist SBOM contains:
...
- `metadata.authors[]` — `Bematist Release Engineering <release@bematist.dev>`.
...
- `metadata.supplier` — Bematist legal entity per release.
...
- `vulnerabilities[]` — VEX attestations where Bematist has assessed a known CVE in a transitive dep and asserts not-affected (per CISA "Vulnerability Exploitability eXchange" guidance).
- `signature` — JOSE detached signature over the canonical-JSON-serialised document, signed with the Bematist release Ed25519 key (cosign-equivalent; pinned in `SLSA_PROVENANCE_KEY`).
...
bunx --bun @cyclonedx/cyclonedx-bom \
  --output-format JSON \
  --output-file "$OUT_DIR/bematist-workspaces-${VERSION}.cdx.json" \
  --spec-version 1.5 \
  --package-lock-only
...
for IMG in web ingest worker ingest-sidecar; do
  syft "ghcr.io/bematist/${IMG}:${VERSION}" \
       -o cyclonedx-json \
...
for OS_ARCH in linux-amd64 linux-arm64 darwin-amd64 darwin-arm64 windows-amd64; do
  cp "$OUT_DIR/bematist-workspaces-${VERSION}.cdx.json" \
     "$OUT_DIR/bematist-${VERSION}-${OS_ARCH}.cdx.json"
  ...
  bunx --bun ts-node scripts/sbom/rewrite-binary-meta.ts \
    "$OUT_DIR/bematist-${VERSION}-${OS_ARCH}.cdx.json" \
    --binary-purl "pkg:generic/bematist@${VERSION}?os=${OS_ARCH%-*}&arch=${OS_ARCH##*-}"
done
...
bunx --bun ts-node scripts/sbom/aggregate.ts \
  ...
  --output "$OUT_DIR/bematist-${VERSION}.cdx.json"
...
for SBOM in "$@"; do
  ...
  # 2) Field-completeness assertions Bematist requires beyond the spec minimum.
...
```bash
#!/usr/bin/env bash
# sign.sh — sign each SBOM with cosign sign-blob using the Bematist release key.
...
sbom:
  ...
  steps:
    ...
    - name: Diff against previous release
      run: scripts/sbom/diff.sh dist/sbom/bematist-${GITHUB_REF_NAME}.cdx.json
    - name: Sign SBOMs
      env:
    ...
    - name: Upload SBOM assets to release
      ...
      with:
        files: |
          dist/sbom/bematist-${{ github.ref_name }}.cdx.json
          dist/sbom/bematist-${{ github.ref_name }}.cdx.json.sig
          dist/sbom/bematist-${{ github.ref_name }}.cdx.json.pem
...
```json
{
  ...
  "metadata": {
    ...
    "authors": [
      {
        "name": "Bematist Release Engineering",
        "email": "release@bematist.dev"
      }
    ],
    "component": {
      "bom-ref": "pkg:generic/bematist@0.0.0",
      "type": "application",
      "name": "bematist",
      "version": "0.0.0",
      "description": "Bematist — open-source AI-engineering analytics platform.",
      "supplier": {
        "name": "Bematist",
      ...
      "purl": "pkg:generic/bematist@0.0.0"
    }
  },
  "components": [
    {
  ...
  "dependencies": [
    {
      "ref": "pkg:generic/bematist@0.0.0",
      "dependsOn": [
  ...
  "compositions": [
    {
      "aggregate": "complete",
      "assemblies": ["pkg:generic/bematist@0.0.0"]
    }
  ],
  "vulnerabilities": []
}
...
Per CycloneDX 1.5 §`vulnerabilities`, Bematist publishes VEX statements as part of the SBOM where a known CVE applies to a transitive dependency that Bematist has assessed not-exploitable in our usage profile. Format:
  ...
  "vulnerabilities": [
    {
      ...
      "analysis": {
        ...
        "detail": "Bematist does not invoke the vulnerable code path because <reason>."
      }
    }
  ]
}
...
bunx --bun @cyclonedx/cyclonedx-cli convert \
  --input-file dist/sbom/bematist-${VERSION}.cdx.json \
  --input-format json \
  --output-file dist/sbom/bematist-${VERSION}.spdx.json \
...
Self-host customers running Bematist on their own infrastructure can:

1. Pull the corresponding GitHub Release SBOMs for their installed version.
2. Verify the cosign signature using the Bematist public key documented in `SECURITY.md`.
3. Feed the SBOM into their own SCA / vulnerability scanner (e.g., Grype, Trivy, Anchore Enterprise).
4. Subscribe to the Bematist SECURITY-advisories mailing list to receive VEX-update notifications between releases.

Self-host customers who need an SBOM for **their own modified build** of Bematist (open-source modification per Apache 2.0) regenerate using the provided scripts; the supply-chain integrity claim then attaches to their build, not Bematist's.
