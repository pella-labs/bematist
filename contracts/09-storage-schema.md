...
- 2026-04-17 — D1-03: added `repo_lookup` and `cluster_lookup` projections on `events` (migrations `0008`, `0009`). Additive — primary ORDER BY unchanged per §Projections. `events` table setting `deduplicate_merge_projection_mode = 'rebuild'` added (required for projections on `ReplacingMergeTree`; safe because Redis SETNX is authoritative dedupe per invariant 2).
